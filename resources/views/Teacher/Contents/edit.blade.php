...
@section('breadcrumb')
    ...
    <span class="text-gray-900 font-semibold">Edit Content</span>
@endsection
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="mb-8">
            <h1 class="text-4xl font-black text-gray-900 mb-2">Edit Content</h1>
        ...
        <div class="bg-white rounded-2xl shadow-lg p-8 border-2 border-gray-100">
            <form action="{{ route('teacher.contents.update', [$course, $content]) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <p class="mt-2 text-sm text-gray-600">
                            Ubah urutan content. Total content: {{ $course->contents->count() }}
                        </p>
                        @error('order')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <textarea name="content" id="content" rows="12" required
                                  ...
                                  placeholder="Tulis materi pembelajaran Anda di sini...">{{ old('content', $content->content) }}</textarea>
                        @error('content')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
                        <div class="flex">
                            ...
                            <div class="text-sm text-blue-800">
                                <p class="font-bold mb-1">Content Info:</p>
                                <ul class="space-y-1">
                                    <li>• Created: <strong>{{ $content->created_at->format('d M Y') }}</strong></li>
                                    <li>• Last Updated: <strong>{{ $content->updated_at->diffForHumans() }}</strong></li>
                                    <li>• Current Order: <strong>{{ $content->order }}</strong></li>
                                </ul>
                            </div>
                        </div>
                    </div>
