...
@section('title', 'Create Content - ' . $course->title)
...
<div class="min-h-screen bg-gray-50 py-8">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-8">
            <h1 class="text-4xl font-black text-gray-900 mb-2">Buat Materi Baru</h1>
            <p class="text-lg text-gray-600">Tambahkan materi pembelajaran untuk kursus: <strong>{{ $course->title }}</strong></p>
        </div>

        <div class="bg-white rounded-2xl shadow-lg p-8 border-2 border-gray-100">
            <form action="{{ route('teacher.contents.store', $course) }}" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="mb-6">
                    ...
                    <textarea name="description" id="description" rows="3"
                              ...
                              placeholder="Jelaskan secara singkat apa yang akan dipelajari di materi ini...">{{ old('description') }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div id="content-text-section" class="mb-6" style="display: {{ old('content_type', 'text') == 'text' ? 'block' : 'none' }};">
                    ...
                    <textarea name="content_text" id="content_text" rows="12"
                              ...
                              placeholder="Tulis materi pembelajaran di sini...">{{ old('content_text') }}</textarea>
                    @error('content_text')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div id="content-file-section" class="mb-6" style="display: {{ old('content_type') == 'file' ? 'block' : 'none' }};">
                    <label for="content_file" class="block text-sm font-bold text-gray-700 mb-2">
                        Upload File/Media <span class="text-red-500">*</span>
                    </label>
                ...
                <div class="mb-6">
                    ...
                    @error('order')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="mt-2 text-xs text-gray-500">Materi saat ini: {{ $lastOrder }}. Materi baru akan ditambahkan di urutan {{ $lastOrder + 1 }}</p>
                </div>
...
function toggleContentType() {
    ...
    if (contentType === 'text') {
        ...
        document.getElementById('content_file').required = true;
    }
}
